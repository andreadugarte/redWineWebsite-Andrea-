...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const p = getProducer(params.slug, "zh");
  if (!p) return {};
  return {
    title: tc(p.name),
    description: p.bio[0]?.slice(0, 160),
    alternates: {
      canonical: `/zh/producers/${params.slug}`,
      languages: {
...
export default function ProducerDetailPageZh({ params }: { params: { slug: string } }) {
  return <ProducerDetailView slug={params.slug} locale="zh" />;
}
